    ...
    public class Data : IDisposable
    {
        ...
        public async Task<List<Message>> GetUnsendedMessages()
        {
            ...
            try
            {
                ...
                messages.Load(await new SQLiteCommand($"SELECT [message_id], [from_id], [date], [chat_id], [text] FROM [telegram_messages] where [issended]='False';", Connection).ExecuteReaderAsync());
        ...
        public async void SetToken(int managerId, string token)
        {
            ...
            try
            {
                var command = new SQLiteCommand("UPDATE [telegram_managers] SET [telegramchatid] is NULL, [token] =@token WHERE [managerid]=@managerId; ", Connection);
                command.Parameters.Add("@token", DbType.String);
                command.Parameters["@token"].Value = token;
        ...
        public async Task<DataTable> GetTelegramManagerTableAsync()
        {
            var offices = new DataTable();
            try
            {
                offices.Load(await new SQLiteCommand("SELECT * FROM telegrammanager;", Connection).ExecuteReaderAsync());
            }
            catch (Exception e)
            {
                Log.Add(new Log.LogMessage(Log.MessageType.ERROR, "GetTelegramManagerTableAsync: " + e.Message));
#if DEBUG
                throw;
#endif
            }
        ...
        public async Task<List<int>> GetAllManagerIdsAsync()
        {
            ...
            managerIds.Load(await new SQLiteCommand($"SELECT managerid FROM telegrammanager;", Connection).ExecuteReaderAsync());
        ...
        public async Task<List<Chat>> GetManagerChatsAsync()
        {
            ...
            chatids.Load(await new SQLiteCommand($"SELECT telegramchatid FROM telegrammanager where telegramchatid IS NOT NULL;", Connection).ExecuteReaderAsync());

            foreach (var chatid in chatids.AsEnumerable())
            {
                var chats = new DataTable();
                chats.Load(await new SQLiteCommand($"SELECT id, type, title, username, first_name, last_name FROM telegramchats where id={chatid[0].ToString()};", Connection).ExecuteReaderAsync());
        ...
        public async Task<List<Message>> GetMessagesFromChatAsync(Chat chat)
        {
            ...
            var messages = new DataTable();
            messages.Load(await new SQLiteCommand($"SELECT messageid, senderid, date, chatid, text, issended FROM telegrammessages where chatid={chat.Id};", Connection).ExecuteReaderAsync());

            var users = new DataTable();
            users.Load(await new SQLiteCommand($"SELECT id, first_name, last_name, username FROM telegramuser;", Connection).ExecuteReaderAsync());

            var startDateTime = new DateTime(1970, 1, 1, 0, 0, 0, 0);
            return Enumerable.Select(messages.AsEnumerable(), row => new Telegram.Bot.Types.Message
            {
                MessageId = int.Parse(row[0].ToString()),
        ...
        public async Task<List<Message>> GetAllMessagesAsync()
        {
            ...
            var messages = new DataTable();
            messages.Load(await new SQLiteCommand($"SELECT messageid, senderid, date, chatid, text, issended FROM telegrammessages;", Connection).ExecuteReaderAsync());
        ...
        public async Task<List<Chat>> GetOpenedChatsAsync()
        {

            var chats = new DataTable();
            chats.Load(await new SQLiteCommand($"SELECT id, type, title, username, first_name, last_name FROM telegramchats where isclosed='False' AND isdialogopened='True';", Connection).ExecuteReaderAsync());
